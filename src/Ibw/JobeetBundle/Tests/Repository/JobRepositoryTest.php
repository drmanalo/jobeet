...
class JobRepositoryTest extends WebTestCase {
	...
	public function setUp() {
		...
		$connection = $this->application->getKernel()->getContainer()
		...
		$loader = new \Symfony\Bridge\Doctrine\DataFixtures\ContainerAwareLoader(
				$client->getContainer());
		$loader
				->loadFromDirectory(
						static::$kernel
								->locateResource(
										'@IbwJobeetBundle/DataFixtures/ORM'));
	...
	public function testCountActiveJobs() {
		...
		foreach ($categories as $category) {
			$query = $this->em
					->createQuery(
							'SELECT COUNT(j.id) FROM IbwJobeetBundle:Job j WHERE j.category = :category AND j.expires_at > :date');
			$query->setParameter('category', $category->getId());
			$query->setParameter('date', date('Y-m-d H:i:s', time()));
			$jobs_db = $query->getSingleScalarResult();

			$jobs_rep = $this->em->getRepository('IbwJobeetBundle:Job')
	...
	public function testGetActiveJobs() {
		...
		foreach ($categories as $category) {
			$query = $this->em
					->createQuery(
							'SELECT COUNT(j.id) from IbwJobeetBundle:Job j WHERE j.expires_at > :date AND j.category = :category');
			$query->setParameter('date', date('Y-m-d H:i:s', time()));
			$query->setParameter('category', $category->getId());
			$jobs_db = $query->getSingleScalarResult();

			$jobs_rep = $this->em->getRepository('IbwJobeetBundle:Job')
					->getActiveJobs($category->getId(), null, null);
	...
	public function testGetActiveJob() {
		...
		$job_rep = $this->em->getRepository('IbwJobeetBundle:Job')
		...
		$job_rep = $this->em->getRepository('IbwJobeetBundle:Job')
				->getActiveJob($job_expired->getId());
